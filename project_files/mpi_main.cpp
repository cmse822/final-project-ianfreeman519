    ...
    **************************************************************************/        
    ...
    {   
        ...
        if (ti % outputFrequency == 0 && write_velocities) {
            ...
            int numRequests = 0;  // Will keep track of the number of requests
            ...
            if (rank == 0) {
                // Prepare to receive data from other ranks
                for (int src = 1; src < size; src++) {
                    int start_idx = NG + src * Nx_l;
                    for (int i = 0; i < Nx_l; i++) {
                        int idx = start_idx + i;
                        reqs.push_back(MPI_Request());
                        MPI_Irecv(u_out[idx].data(), u_out[idx].size(), MPI_DOUBLE, src, src * 10000 + i, MPI_COMM_WORLD, &reqs.back());
                        reqs.push_back(MPI_Request());
                        MPI_Irecv(v_out[idx].data(), v_out[idx].size(), MPI_DOUBLE, src, src * 10000 + i + Nx, MPI_COMM_WORLD, &reqs.back());
                    }
                }
                numRequests = reqs.size();
            }

            // Non-root ranks send their data
            if (rank != 0) {
                for (int i = NG; i < Nx_l + NG; i++) {
                    // Setting the send id to be large enough that it never gets touched by the recv ids
                    int send_id_u = rank * 10000 + (i - NG);
                    int send_id_v = rank * 10000 + (i - NG) + Nx;
                    MPI_Request req;
                    MPI_Isend(u[i].data(), u[i].size(), MPI_DOUBLE, 0, send_id_u, MPI_COMM_WORLD, &req);
                    reqs.push_back(req);
                    MPI_Isend(v[i].data(), v[i].size(), MPI_DOUBLE, 0, send_id_v, MPI_COMM_WORLD, &req);
                    reqs.push_back(req);
                }
                numRequests = reqs.size();
            }
            ...
            std::vector<MPI_Status> statuses(numRequests);
            MPI_Waitall(numRequests, reqs.data(), statuses.data());

            // Root processes the data
            if (rank == 0) {
                for (int j=NG; j<Ny+NG; j++){
                ...
                writeToCsv(u_out, "output/u" + std::to_string(ti) + ".csv");
                writeToCsv(v_out, "output/v" + std::to_string(ti) + ".csv");
                writeToCsv(Re_out, "output/Re" + std::to_string(ti) + ".csv");
            }
        }
    ...
    if (rank == 0) {
        write1DToCsv(inner_loop_timer, "timings/timer" + std::to_string(size) + ".csv");
    }
    MPI_Finalize();
    return 0;
}