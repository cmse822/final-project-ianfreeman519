...
int main(int argc, char* argv[]) {
    ...
    **************************************************************************/        
    ...
    if (write_velocities) {
        writeToCsv(u, "output_omp/u_" + directory_name + "_" + std::to_string(ti) + ".csv");
        writeToCsv(v, "output_omp/v_" + directory_name + "_" + std::to_string(ti) + ".csv");
        writeToCsv(Re, "output_omp/Re_" + directory_name + "_" + std::to_string(ti) + ".csv");
    }
    ...
    {   
        ...
        // Actual iteration
        #pragma omp parallel for collapse(2) schedule(dynamic) shared(u, v, u_new, v_new, Re, rho, L, mu, dx, dy, Nx, Ny, NG, gx, gy, p, dt) default(none) private(ut1, ut2, ut3, ut4, ut5, vt1, vt2, vt3, vt4, vt5, invRe, sx, sy)
        // #pragma omp parallel for schedule(static) shared(u, v, u_new, v_new, Re, rho, L, mu, dx, dy, Nx, Ny, NG, gx, gy, p, dt) default(none) private(ut1, ut2, ut3, ut4, ut5, vt1, vt2, vt3, vt4, vt5, invRe, sx, sy)
        for (int j=NG; j<Ny+NG; j++){
            for (int i=NG; i<Nx+NG; i++){
    ...
    write1DToCsv(inner_loop_timer, "output_omp/timer_" + directory_name + "_" + std::to_string(omp_get_max_threads()) + ".csv");
    return 0;
}