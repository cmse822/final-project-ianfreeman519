...
InputReader readInput(const std::string& filename) {
    ...
    while (std::getline(inputFile, line)) {     // keep reading the file, storing each line in 'line'
        ...
        if (std::getline(lineStream, key, '=')) {   // reads line of input until '=' is hit, stores it into 'key'
            ...
            if (key == "Nx") input.Nx = static_cast<int>(value);        // Number of nodes in x-direction
            else if (key == "Ny") input.Ny = static_cast<int>(value);   // Number of nodes in y-direction
            else if (key == "xmin") input.xmin = value;                 // minimum x value (m)
            else if (key == "xmax") input.xmax = value;                 // maximum x value (m)
            else if (key == "ymin") input.ymin = value;                 // minimum y value (m)
            else if (key == "ymax") input.ymax = value;                 // maximum y value (m)
            // block_size is the edge length of the MPI blocks that will be passed to different processors
            else if (key == "block_size") input.block_size = static_cast<int>(value);
            else if (key == "dt") input.dt = value;                     // simulation dt (s)
            else if (key == "t_max") input.t_max = value;               // simulation stopping time (s)
            else if (key == "viscosity") input.viscosity = value;       // viscocity of fluid (Pa*s)
            else if (key == "density") input.density = value;           // density of fluid (kg/m3)
        }
    }
    ...
    if (input.density <= 0.0 || input.viscosity <= 0.0)
        throw std::runtime_error("density and viscocity must be positive and nonzero");
    ...
    printf("\nviscosity = %f | density = %f", input.viscosity, input.density);
    printf("\n");
